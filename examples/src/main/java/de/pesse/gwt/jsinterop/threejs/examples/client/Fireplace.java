...
public class Fireplace
{
	...
	private void init()
	{
		if (canvas != null)
		{
			renderer = new WebGLRenderer(new WebGLRendererParameters.Builder().canvas(canvas).build());
		} else
		{
		...
		Mesh ground = new Mesh(new PlaneBufferGeometry(10, 10),
				new MeshPhongMaterial(new MeshPhongMaterialParameters.Builder().map(groundColor).normalMap(groundNormal)
						.normalScale(new Vector2(0.8, 0.8)).build()));
		...
		callback = new AnimationFrameCallback()
		{
			...
			public void execute()
			{
				...
				cam.position.set(Math.sin(elapsed * 0.1) * 4, Math.sin(elapsed * 0.5) * 1 + 2,
